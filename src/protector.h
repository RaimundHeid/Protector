...
typedef enum { DARK, LIGHT, ALL } SquareColor;

typedef enum { RESULT_UNKNOWN, RESULT_WHITE_WINS, RESULT_DRAW, RESULT_BLACK_WINS } GameResult;

typedef enum { TS_FALSE = 0, TS_UNKNOWN = 1, TS_TRUE = 2 } TriStateType;
...
typedef enum { NO_CASTLINGS = 0, WHITE_00 = 1, WHITE_000 = 2, BLACK_00 = 4, BLACK_000 = 8 } Castlings;
...
#define colorRank(color, square) (Rank)((color) == WHITE ? rank(square) : 7 - rank(square))
#define squareIsValid(square) ((square) >= A1 && (square) <= H8)
#define getSquare(file, rank) ((Square)((file) + ((rank) << 3)))
#define getFlippedSquare(square) (getSquare(file(square), (RANK_8 - rank(square))))
#define getHflippedSquare(square) (getSquare((FILE_H - file(square)), rank(square)))
