...
static const int sfBalanceValue[16] = {
    ...
    [WHITE_KING] = 0,
    [BLACK_KING] = 0,
    [WHITE_QUEEN] = SFVAL_QUEEN,
    [BLACK_QUEEN] = -SFVAL_QUEEN,
    [WHITE_ROOK] = SFVAL_ROOK,
    [BLACK_ROOK] = -SFVAL_ROOK,
    [WHITE_BISHOP] = SFVAL_BISHOP,
    [BLACK_BISHOP] = -SFVAL_BISHOP,
    [WHITE_KNIGHT] = SFVAL_KNIGHT,
    [BLACK_KNIGHT] = -SFVAL_KNIGHT,
    [WHITE_PAWN] = SFVAL_PAWN,
...
static const int sfMaterialValue[16] = {
    ...
    [WHITE_KNIGHT] = SFVAL_KNIGHT,
    [BLACK_KNIGHT] = SFVAL_KNIGHT,
    [WHITE_PAWN] = SFVAL_PAWN_MATERIAL,
    [BLACK_PAWN] = SFVAL_PAWN_MATERIAL,
};
...
{
    const Bitboard king = getKingMoves(square) & minValue[position->king[attackerColor]];
    ...
    const Bitboard pawns =
        getPawnCaptures((Piece)(PAWN | opponent(attackerColor)), square, position->piecesOfType[PAWN | attackerColor]);

    ortho &= (position->piecesOfType[QUEEN | attackerColor] | position->piecesOfType[ROOK | attackerColor]);
    dia &= (position->piecesOfType[QUEEN | attackerColor] | position->piecesOfType[BISHOP | attackerColor]);
...
{
    return squaresBehind[targetSquare][viewPoint] & getMagicBishopMoves(targetSquare, position->allPieces);
}
...
{
    return squaresBehind[targetSquare][viewPoint] & getMagicRookMoves(targetSquare, position->allPieces);
}
...
void appendMoveToPv(const PrincipalVariation *oldPv, PrincipalVariation *newPv, const Move move)
{
    ...
    if (movesToCopy > 0) {
        memmove((void *)&newPv->move[1], (void *)&oldPv->move[0], movesToCopy * sizeof(UINT16));
    }
}
...
{
    return position->piecesOfColor[color] & ~(position->piecesOfType[PAWN | color] | minValue[position->king[color]]);
}
...
{
    return position->piecesOfColor[color] & ~position->piecesOfType[PAWN | color];
}
...
{
    return position->piecesOfType[ROOK | color] | position->piecesOfType[QUEEN | color];
}
...
int getMinimalDistance(const Position *position, const Square origin, const Piece piece)
{
    ...
    for (distance = 1; distance <= 7; distance++) {
        if ((squaresInDistance[distance][origin] & position->piecesOfType[piece]) != EMPTY_BITBOARD) {
            return distance;
        }
    }
...
int getMinimalTaxiDistance(const Position *position, const Square origin, const Piece piece)
{
    ...
    for (distance = 1; distance <= 14; distance++) {
        if ((squaresInTaxiDistance[distance][origin] & position->piecesOfType[piece]) != EMPTY_BITBOARD) {
            return distance;
        }
    }
...
void initializePosition(Position *position)
{
    ...
    if ((position->castlingRights & WHITE_00) &&
        (position->piece[E1] != WHITE_KING || position->piece[H1] != WHITE_ROOK)) {
        position->castlingRights -= WHITE_00;
    }

    if ((position->castlingRights & WHITE_000) &&
        (position->piece[E1] != WHITE_KING || position->piece[A1] != WHITE_ROOK)) {
        position->castlingRights -= WHITE_000;
    }

    if ((position->castlingRights & BLACK_00) &&
        (position->piece[E8] != BLACK_KING || position->piece[H8] != BLACK_ROOK)) {
        position->castlingRights -= BLACK_00;
    }

    if ((position->castlingRights & BLACK_000) &&
        (position->piece[E8] != BLACK_KING || position->piece[A8] != BLACK_ROOK)) {
    ...
    position->numberOfPieces[WHITE] = getNumberOfSetSquares(position->piecesOfColor[WHITE]);
    position->numberOfPieces[BLACK] = getNumberOfSetSquares(position->piecesOfColor[BLACK]);
    position->numberOfPawns[WHITE] = getNumberOfSetSquares(position->piecesOfType[WHITE_PAWN]);
    position->numberOfPawns[BLACK] = getNumberOfSetSquares(position->piecesOfType[BLACK_PAWN]);

    position->materialBalance = SFVAL_PAWN * (position->numberOfPawns[WHITE] - position->numberOfPawns[BLACK]) +
    ...
    position->materialCount = SFVAL_PAWN_MATERIAL * (position->numberOfPawns[WHITE] + position->numberOfPawns[BLACK]) +
                              SFVAL_KNIGHT * (getNumberOfSetSquares(position->piecesOfType[WHITE_KNIGHT]) +
                                              getNumberOfSetSquares(position->piecesOfType[BLACK_KNIGHT])) +
                              SFVAL_BISHOP * (getNumberOfSetSquares(position->piecesOfType[WHITE_BISHOP]) +
                                              getNumberOfSetSquares(position->piecesOfType[BLACK_BISHOP])) +
...
void flipPosition(Position *position)
{
    ...
    for (square = A1; square <= H4; square++) {
        ...
        position->piece[square] =
            (position->piece[flippedSquare] == NO_PIECE ? NO_PIECE : (Piece)(position->piece[flippedSquare] ^ BLACK));
        position->piece[flippedSquare] = (piece == NO_PIECE ? NO_PIECE : (Piece)(piece ^ BLACK));
    }
}
...
Bitboard getInterestedPieces(const Position *position, const Square square, const Color attackerColor)
{
    ...
    Bitboard knights = getKnightMoves(square);
    Bitboard pawns = getInterestedPawns(attackerColor, square, position->allPieces);
...
int makeMove(Variation *variation, const Move move)
{
    ...
    if (pieceType(movingPiece) == PAWN) {
        ...
        if (distance(from, to) == 2) {
            ...
            if (getDirectAttackers(position, from, passiveColor, position->allPieces) != EMPTY_BITBOARD ||
                getDirectAttackers(position, rookTo, passiveColor, position->allPieces) != EMPTY_BITBOARD) {
...
void unmakeLastMove(Variation *variation)
{
    ...
    const Square to = getToSquare(move);
    const Piece newPiece = getNewPiece(move);
    const Color activeColor = position->activeColor = opponent(position->activeColor);
    ...
    if (plyInfo->captured != NO_PIECE) {
    ...
    } else if (to == plyInfo->enPassantSquare && pieceType(position->piece[from]) == PAWN) {
        const Color passiveColor = opponent(activeColor);

        position->piece[plyInfo->restoreSquare1] = plyInfo->restorePiece1;
        setSquare(position->piecesOfType[plyInfo->restorePiece1], plyInfo->restoreSquare1);
        position->numberOfPieces[passiveColor]++;
        position->numberOfPawns[passiveColor]++;
        ...
        setSquare(position->piecesOfType[plyInfo->restorePiece1], plyInfo->restoreSquare1);
        clearSquare(position->piecesOfType[plyInfo->restorePiece1], plyInfo->restoreSquare2);
    }
...
bool moveIsCheck(const Move move, const Position *position)
{
    ...
    if (testSquare(generalMoves[QUEEN][kingSquare], from) &&
        (position->allPieces & squaresBetween[kingSquare][from]) == EMPTY_BITBOARD &&
        ...
        if (rank(from) == rank(kingSquare) || file(from) == file(kingSquare)) {
            batteryPieces = squaresBehind[from][kingSquare] &
                            (position->piecesOfType[ROOK | color] | position->piecesOfType[QUEEN | color]);
        } else {
            batteryPieces = squaresBehind[from][kingSquare] &
                            (position->piecesOfType[BISHOP | color] | position->piecesOfType[QUEEN | color]);
        }

        ITERATE_BITBOARD(&batteryPieces, square)
        {
            if ((squaresBetween[square][kingSquare] & position->allPieces) == minValue[from]) {
                return TRUE;
            }
        }
    }
    ...
    if (piece & PP_SLIDING_PIECE) {
        return (bool)(testSquare(generalMoves[pieceType(piece)][to], kingSquare) &&
                      (position->allPieces & squaresBetween[kingSquare][to]) == EMPTY_BITBOARD);
    } else if (pieceType(piece) == KNIGHT) {
        return (testSquare(generalMoves[KNIGHT][kingSquare], to) != EMPTY_BITBOARD);
        ...
        if (newPiece == NO_PIECE) {
            ...
            if (to == position->enPassantSquare) {
                ...
                if (testSquare(generalMoves[QUEEN][kingSquare], captureSquare) &&
                    (squaresBetween[captureSquare][kingSquare] & position->allPieces) == EMPTY_BITBOARD) {
                    const Color color = position->activeColor;
                    Bitboard batteryPieces;
                    const Bitboard blockers =
                        minValue[to] | (position->allPieces & maxValue[captureSquare] & maxValue[from]);
                    Square square;

                    if (rank(captureSquare) == rank(kingSquare) || file(captureSquare) == file(kingSquare)) {
                        batteryPieces = squaresBehind[captureSquare][kingSquare] &
                                        (position->piecesOfType[ROOK | color] | position->piecesOfType[QUEEN | color]);
                    } else {
                        batteryPieces =
                            squaresBehind[captureSquare][kingSquare] &
                            (position->piecesOfType[BISHOP | color] | position->piecesOfType[QUEEN | color]);
                    }

                    ITERATE_BITBOARD(&batteryPieces, square)
                    {
                        if ((squaresBetween[kingSquare][square] & blockers) == EMPTY_BITBOARD) {
                            return TRUE;
                        }
                    }
                }
            ...
            if (newPiece & PP_SLIDING_PIECE) {
                ...
                return (bool)(testSquare(generalMoves[newPiece][to], kingSquare) &&
                              (blockers & squaresBetween[kingSquare][to]) == EMPTY_BITBOARD);
            } else {
                return (testSquare(generalMoves[KNIGHT][kingSquare], to) != EMPTY_BITBOARD);
            }
        }
    } else if (pieceType(piece) == KING && (from == E1 || from == E8) &&
               (to == G1 || to == C1 || to == G8 || to == C8)) {
        const Bitboard blockers = position->allPieces & maxValue[from];
        const int rookSquare = (int)((to + from) / 2);

        return (bool)(testSquare(generalMoves[ROOK][rookSquare], kingSquare) &&
                      (blockers & squaresBetween[kingSquare][rookSquare]) == EMPTY_BITBOARD);
    }
...
int checkConsistency(const Position *position)
{
    ...
    assert(position->enPassantSquare == NO_SQUARE || squareIsValid(position->enPassantSquare));
    ...
    assert((position->piecesOfColor[WHITE] ^ position->piecesOfColor[BLACK]) == position->allPieces);
    ...
    assert(testSquare(position->piecesOfType[WHITE_KING], position->king[WHITE]));
    assert(getNumberOfSetSquares(position->piecesOfType[WHITE_KING]) == 1);
    assert(testSquare(position->piecesOfType[BLACK_KING], position->king[BLACK]));
    ...
    assert(numPieces[WHITE] == getNumberOfSetSquares(position->piecesOfColor[WHITE]));
    assert(numPieces[BLACK] == position->numberOfPieces[BLACK]);
    assert(numPieces[BLACK] >= 1);
    assert(numPieces[BLACK] <= 16);
    assert(numPieces[BLACK] == getNumberOfSetSquares(position->piecesOfColor[BLACK]));

    assert(numPawns[WHITE] == position->numberOfPawns[WHITE]);
    assert(numPawns[WHITE] >= 0);
    assert(numPawns[WHITE] <= 8);
    assert(numPawns[WHITE] == getNumberOfSetSquares(position->piecesOfType[WHITE_PAWN]));
    assert(numPawns[BLACK] == position->numberOfPawns[BLACK]);
    assert(numPawns[BLACK] >= 0);
    assert(numPawns[BLACK] <= 8);
    assert(numPawns[BLACK] == getNumberOfSetSquares(position->piecesOfType[BLACK_PAWN]));

#ifndef NDEBUG
    {
        const int expectedBalance = SFVAL_PAWN * (numPawns[WHITE] - numPawns[BLACK]) +
                                    SFVAL_KNIGHT * (getNumberOfSetSquares(position->piecesOfType[WHITE_KNIGHT]) -
                                                    getNumberOfSetSquares(position->piecesOfType[BLACK_KNIGHT])) +
                                    SFVAL_BISHOP * (getNumberOfSetSquares(position->piecesOfType[WHITE_BISHOP]) -
                                                    getNumberOfSetSquares(position->piecesOfType[BLACK_BISHOP])) +
                                    ...
                                    SFVAL_QUEEN * (getNumberOfSetSquares(position->piecesOfType[WHITE_QUEEN]) -
                                                   getNumberOfSetSquares(position->piecesOfType[BLACK_QUEEN]));
        const int expectedCount = SFVAL_PAWN_MATERIAL * (numPawns[WHITE] + numPawns[BLACK]) +
                                  SFVAL_KNIGHT * (getNumberOfSetSquares(position->piecesOfType[WHITE_KNIGHT]) +
                                                  getNumberOfSetSquares(position->piecesOfType[BLACK_KNIGHT])) +
                                  SFVAL_BISHOP * (getNumberOfSetSquares(position->piecesOfType[WHITE_BISHOP]) +
                                                  getNumberOfSetSquares(position->piecesOfType[BLACK_BISHOP])) +
...
bool positionIsLegal(const Position *position)
{
    ...
    if ((position->piecesOfColor[WHITE] ^ position->piecesOfColor[BLACK]) != position->allPieces) {
        return FALSE;
    }
    ...
    if (testSquare(position->piecesOfType[WHITE_KING], position->king[WHITE]) == FALSE) {
        return FALSE;
    }
    ...
    if (testSquare(position->piecesOfType[BLACK_KING], position->king[BLACK]) == FALSE) {
        return FALSE;
    }
...
bool positionsAreIdentical(const Position *position1, const Position *position2)
{
    ...
    if (position1->activeColor != position2->activeColor || position1->castlingRights != position2->castlingRights ||
    ...
    if (position1->halfMoveClock != position2->halfMoveClock || position1->moveNumber != position2->moveNumber) {
...
int initializeModulePosition(void)
{
    ...
    ITERATE (square) {
        remainingCastlings[square] = (WHITE_00 | WHITE_000 | BLACK_00 | BLACK_000);
...
{
    return SFVAL_PAWN * (getNumberOfSetSquares(pos->piecesOfType[WHITE_PAWN]) -
...
{
    return SFVAL_PAWN_MATERIAL * (getNumberOfSetSquares(pos->piecesOfType[WHITE_PAWN]) +
                                  getNumberOfSetSquares(pos->piecesOfType[BLACK_PAWN])) +
           SFVAL_KNIGHT * (getNumberOfSetSquares(pos->piecesOfType[WHITE_KNIGHT]) +
                           getNumberOfSetSquares(pos->piecesOfType[BLACK_KNIGHT])) +
           SFVAL_BISHOP * (getNumberOfSetSquares(pos->piecesOfType[WHITE_BISHOP]) +
...
static int testMaterialCountersGeneric(MoveFuncPos moveFunc, const char *moveTypeName)
{
    struct TestCase {
        ...
        {"r1bqkbnr/pppp1ppp/2n5/1B2p3/4P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 0 1", getOrdinaryMove(B5, C6), "Capture Bxc6"},
        /* En passant */
        {"rnbqkbnr/pppp1ppp/8/4pP2/8/8/PPPPP1PP/RNBQKBNR w KQkq e6 0 1", getOrdinaryMove(F5, E6), "En passant fxe6"},
        /* Promotion */
