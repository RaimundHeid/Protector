...
static long getLongUciValue(const char *uciString, const char *name, int defaultValue)
{
    long value;
...
static void getStringUciValue(const char *uciString, const char *name, char *stringValue, size_t bufferSize)
{
    const char *nameStart = getUciToken(uciString, name);
...
static void getUciNamedValue(const char *uciString, char *name, size_t nameSize, char *value, size_t valueSize)
{
    ...
    if (nameTagStart != 0 && valueTagStart != 0 && nameTagStart < valueTagStart) {
 ...
 *
 ******************************************************************************/
static void getTokenByNumber(const char *command, int paramNumber, char *buffer)
{
    int paramCount = 0;
    char currentChar;
...
static int getCalculationTime(TimecontrolData *data)
{
    ...
    if (data->movesToGo > 0) {
        ...
        return (data->incrementTime > 0 ? data->incrementTime + data->restTime / movesToGo
...
{
    const char *format = "info depth %d seldepth %d time %.0f nodes %llu nps %.0f score %s %s tbhits %lu %s pv %s";
    ...
    if (time >= 250 || sendAnyway) {
        ...
        snprintf(pvBuffer, sizeof(pvBuffer), format, var->iteration, var->selDepth, time, nodeCount, nps, scoreBuffer,
                 scoreTypeBuffer, var->tbHits, multiPvBuffer, pvMovesBuffer);
...
static void reportBaseMoveUpdate(const Variation *var)
{
    ...
    if (time >= 500) {
        ...
        sendToUciNonDebug("info depth %d seldepth %d currmove %s currmovenumber %d", var->iteration, var->selDepth,
                          movetext, var->numberOfCurrentBaseMove);
    }
}
...
static void reportStatisticsUpdate(Variation *var)
{
    ...
    const double time = getTimestamp() - var->startTime;
    const double nps = (nodeCount / max((double)0.001, (time / 1000.0)));
    const double hashUsage = ((double)getSharedHashtable()->entriesUsed * 1000.0) /
                             (max((double)1.0, (double)getSharedHashtable()->tableSize));

    sendToUciNonDebug("info time %0.f nodes %llu nps %.0f hashfull %.0f tbhits %lu", time, nodeCount, nps, hashUsage,
                      var->tbHits);
...
static void sendBestmoveInfo(Variation *var)
{
    ...
    if (moveIsLegal(&var->startPosition, var->bestBaseMove)) {
        ...
        *tmp = *var;
        ...
        if (status.ponderingMove != NO_MOVE && moveIsLegal(&tmp->singlePosition, status.ponderingMove)) {
            char ponderMoveBuffer[16];

            getGuiMoveString(status.ponderingMove, ponderMoveBuffer, sizeof(ponderMoveBuffer));
            sendToUciNonDebug("bestmove %s ponder %s", moveBuffer, ponderMoveBuffer);
        } else {
...
static int getIntValue(const char *value, int minValue, int defaultValue, int maxValue)
{
...
static void sendUciSpinOption(const char *name, const int defaultValue, int minValue, int maxValue)
{
    sendToUciNonDebug("option name %s type spin default %d min %d max %d", name, defaultValue, minValue, maxValue);
}

void addHashentry(Hashentry *entry)
{
    setHashentry(getSharedHashtable(), entry->key, getHashentryValue(entry), getHashentryImportance(entry),
                 getHashentryMove(entry), getHashentryFlag(entry), getHashentryStaticValue(entry));
}
...
static int processUciCommand(const char *command)
{
    ...
    if (strcmp(buffer, "uci") == 0) {
        ...
        sendToUciNonDebug("option name Hash type spin default 16 min 8 max 65536");
        sendToUciNonDebug("option name SyzygyPath type string default <empty>");
    ...
    if (strcmp(buffer, "setoption") == 0) {
        ...
        if (strcmp(name, USN_NT) == 0) {
            const unsigned int numThreads = (unsigned int)getIntValue(value, 1, 1, MAX_THREADS);
    ...
    if (strcmp(buffer, "ponderhit") == 0) {
        ...
        if (status.engineIsActive) {
            if (getCurrentVariation()->terminateSearchOnPonderhit && getCurrentVariation()->failingLow == FALSE) {
                prepareSearchAbort();
            } else {
                unsetPonderMode();
    ...
    if (strcmp(buffer, "go") == 0) {
        ...
        if (getUciToken(command, "depth") != 0) {
            ...
            status.timecontrolData[WHITE].restTime = status.timecontrolData[BLACK].restTime = -1;
            status.timecontrolData[WHITE].incrementTime = status.timecontrolData[BLACK].incrementTime =
            ...
            status.timecontrolData[WHITE].restTime = getLongUciValue(command, "wtime", 1000);
            status.timecontrolData[WHITE].incrementTime = getLongUciValue(command, "winc", 0);
            status.timecontrolData[BLACK].restTime = getLongUciValue(command, "btime", 1000);
            status.timecontrolData[BLACK].incrementTime = getLongUciValue(command, "binc", 0);
            status.timecontrolData[WHITE].numberOfMovesPlayed = status.timecontrolData[BLACK].numberOfMovesPlayed =
                numMovesPlayed;

            if (movesToGo > 0) {
                status.timecontrolData[WHITE].movesToGo = status.timecontrolData[BLACK].movesToGo = movesToGo;
            } else {
                status.timecontrolData[WHITE].movesToGo = status.timecontrolData[BLACK].movesToGo = 0;
            }
        }
...
int initializeModuleUci(void)
{
    ...
    status.timecontrolData[BLACK].incrementTime = status.timecontrolData[WHITE].incrementTime;
    status.timecontrolData[BLACK].numberOfMovesPlayed = 0;
    status.timecontrolData[BLACK].restTime = status.timecontrolData[WHITE].restTime;
    deletePonderResult();
...
static int testUciTokenizer(void)
{
    ...
    const char *uciString = "setoption name\tSyzygyPath    value  \t  C:\\chess\\tablebases   time  641273423";
...
static int testHashUpdate(void)
{
    ...
    Hashentry entry = constructHashEntry(hashKey, value, staticValue, importance, bestMove, date, flag);
