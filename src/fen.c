...
void getFen(const Position *position, char *fen, size_t bufferSize)
{
    ...
    for (rank = RANK_8; rank >= RANK_1; rank--) {
        ...
        for (file = FILE_A; file <= FILE_H; file++) {
            ...
            if (position->piece[square] != NO_PIECE) {
                if (emptyCount > 0) {
                    if (p < bufferSize - 1)
                        fen[p++] = (char)('0' + emptyCount);
                    emptyCount = 0;
                }

                if (p < bufferSize - 1)
                    fen[p++] = pieceToken[position->piece[square]];
            } else {
                emptyCount++;
            }
        }
    ...
    if (p < bufferSize - 1)
        fen[p++] = ' ';
    if (p < bufferSize - 1)
        fen[p++] = (position->activeColor == WHITE ? 'w' : 'b');
    if (p < bufferSize - 1)
        fen[p++] = ' ';
...
int testModuleFen(void)
{
    ...
    assert(position.castlingRights == (WHITE_00 | WHITE_000 | BLACK_00 | BLACK_000));
