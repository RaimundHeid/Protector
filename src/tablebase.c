...
int probeTablebaseWDL(const Position *position)
{
    ...
    unsigned res = tb_probe_wdl(
        ...
        position->piecesOfType[WHITE_PAWN] | position->piecesOfType[BLACK_PAWN], position->halfMoveClock,
        0, // castling
        (position->enPassantSquare == NO_SQUARE ? 0 : position->enPassantSquare), (position->activeColor == WHITE));
...
int probeTablebase(const Position *position)
{
    ...
    if (res == TABLEBASE_ERROR)
        return TABLEBASE_ERROR;

    switch (res) {
    case 2:
        return -VALUE_MATED - MAX_DEPTH - 1;
    case 1:
        return 1; // Slight advantage for cursed win
    case 0:
        return 0;
    case -1:
        return -1; // Slight disadvantage for blessed loss
    case -2:
        return VALUE_MATED + MAX_DEPTH + 1;
    default:
        return TABLEBASE_ERROR;
...
int probeTablebaseDTZ(const Position *position, Move *move)
{
    ...
    unsigned res =
        tb_probe_root(position->piecesOfColor[WHITE], position->piecesOfColor[BLACK],
                      position->piecesOfType[WHITE_KING] | position->piecesOfType[BLACK_KING],
                      position->piecesOfType[WHITE_QUEEN] | position->piecesOfType[BLACK_QUEEN],
                      position->piecesOfType[WHITE_ROOK] | position->piecesOfType[BLACK_ROOK],
                      position->piecesOfType[WHITE_BISHOP] | position->piecesOfType[BLACK_BISHOP],
                      ...
                      (position->activeColor == WHITE), NULL);
    ...
    if (wdl == TB_WIN)
        return -VALUE_MATED - MAX_DEPTH - 100 - dtz;
