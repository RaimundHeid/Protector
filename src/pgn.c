...
static int scanForIndex(PGNFile *pgnfile, int state, char buffer[], size_t bufsize, size_t offset)
{
    size_t i = 0;
        ...
        case '[':
            ...
            if (state == 2) {
                if (pgnfile->numGames + 1 == pgnfile->indexSize) {
                    ...
                    newIndex = (long *)realloc(pgnfile->index, pgnfile->indexSize * sizeof(long));
                    if (newIndex == NULL) {
                        return 0;
                    }
...
static Gamemove *getGamemove(const Position *position, Gamemove *previousMove, PGNGame *game)
{
...
static Move parsePawnMove(const Position *position, const char *moveText)
{
    ...
    if (textLength >= 4 && moveText[1] == 'x') {
        ...
        from = (Square)(getSquare(moveText[0] - 'a', moveText[3] - '1') - pawnStep);
    } else {
...
static Move parseCastlingMove(const Position *position, const char *moveText)
{
    ...
    return getPackedMove(getSquare(FILE_E, rank), getSquare(file, rank), NO_PIECE);
}

static Move parsePieceMove(const Position *position, const char *moveText, const PieceType pieceType)
{
    Square from = NO_SQUARE, to = NO_SQUARE;
    Piece newPiece = NO_PIECE;
    size_t textLength = strlen(moveText);
    ...
    to = getSquare(moveText[textLength - 2] - 'a', moveText[textLength - 1] - '1');
    ...
    if (fromFile != -1 && fromRank != -1) {
        ...
        candidates = getDirectAttackers(position, to, position->activeColor, position->allPieces) &
...
static Move parsePGNMove(const char *moveText, Position *position)
{
    ...
    while (i < 15 && moveText[i] != 0 && moveText[i] != '+' && moveText[i] != '#' && moveText[i] != '!' &&
           moveText[i] != '?' && moveText[i] != ')' && !isspace((int)moveText[i])) {
...
int appendMove(PGNGame *game, const Move move)
{
    ...
    if (moveIsLegal(&variation->singlePosition, move)) {
        Gamemove *newMove = getGamemove(&variation->singlePosition, game->lastMove, game);
...
static Gamemove *parseMoveText(const char *pgnMoveText, size_t *offset, const Position *position, PGNGame *game)
{
    ...
    while (variationTerminated == FALSE && (currentChar = pgnMoveText[(*offset)++]) != '\0') {
        ...
        if (currentChar == '(') {
            ...
            if (lastMove != 0) {
                addAlternativeMove(lastMove, parseMoveText(pgnMoveText, offset, &lastMove->position, game));
            }
        }
        ...
        if (currentChar == '$') {
            ...
            if (lastMove != 0) {
                if (lastMove->glyphs != 0) {
                    ...
                    char *newGlyphs = realloc(lastMove->glyphs, strlen(lastMove->glyphs) + strlen(glyph) + 2);
                    if (newGlyphs != NULL) {
        ...
        if (strchr("KQRBNOabcdefgh", currentChar) != 0) {
            ...
            if (moveIsLegal(&variation->singlePosition, move)) {
                Gamemove *newMove = getGamemove(&variation->singlePosition, lastMove, game);
            ...
            while (isspace((int)pgnMoveText[*offset]) == FALSE && pgnMoveText[*offset] != ')') {
                (*offset)++;
            }
        }
    }
...
static Gamemove *parseMoveSection(const char *pgnMoveText, const Position *position, PGNGame *game)
{
    size_t offset = 0;
...
static void parseRoasterValue(const char *pgn, const char *name, char value[PGN_ROASTERLINE_SIZE])
{
...
static void parseRoasterValues(const char *pgn, PGNGame *pgngame)
{
    ...
    if (moves != 0) {
        ...
        if ((pgngame->moveText = malloc(movesLength + 1)) != 0) {
            ...
            pgngame->firstMove = parseMoveSection(pgngame->moveText, &variation->singlePosition, pgngame);
        }
    }
    free(variation);
}
...
static int examineAlternativeMoves(Variation *variation, const Move move)
{
    ...
    Square square;
    Bitboard candidates =
        getDirectAttackers(position, to, position->activeColor, position->allPieces) & position->piecesOfType[piece];
...
void generateMoveText(Variation *variation, const Move move, char *pgnMove, size_t bufferSize)
{
    ...
    snprintf(pgnMove, bufferSize, "%s%s%s%s%s%s", pieceSign, origin, captureSign, destination, promotionSign,
             checkSign);
}
...
static char *generateMoveSection(Gamemove *gamemove, const char *result)
{
    ...
    while (gamemove != 0) {
        ...
        generateMoveText(variation, getPackedMove(gamemove->from, gamemove->to, gamemove->newPiece), moveBuffer,
                         sizeof(moveBuffer));

        if (gamemove->position.activeColor == WHITE) {
            appendToString(&moveText, " %d. %s", gamemove->position.moveNumber, moveBuffer);
        } else {
...
static char *generateRoasterLine(const char *tag, const char *tagValue, char buffer[PGN_ROASTERLINE_SIZE])
{
    if (tagValue[0] != '\0') {
...
static char *generateRoaster(const PGNGame *pgngame)
{
    ...
    appendToString(&string, generateRoasterLine("Event", pgngame->event, tagbuffer));
    appendToString(&string, generateRoasterLine("Site", pgngame->site, tagbuffer));
    appendToString(&string, generateRoasterLine("Date", pgngame->date, tagbuffer));
    appendToString(&string, generateRoasterLine("Round", pgngame->round, tagbuffer));
    appendToString(&string, generateRoasterLine("White", pgngame->white, tagbuffer));
    appendToString(&string, generateRoasterLine("Black", pgngame->black, tagbuffer));
    appendToString(&string, generateRoasterLine("Result", pgngame->result, tagbuffer));
    appendToString(&string, generateRoasterLine("SetUp", pgngame->setup, tagbuffer));
    appendToString(&string, generateRoasterLine("FEN", pgngame->fen, tagbuffer));
    appendToString(&string, generateRoasterLine("WhiteTitle", pgngame->whiteTitle, tagbuffer));
    appendToString(&string, generateRoasterLine("BlackTitle", pgngame->blackTitle, tagbuffer));
    appendToString(&string, generateRoasterLine("WhiteElo", pgngame->whiteElo, tagbuffer));
    appendToString(&string, generateRoasterLine("BlackElo", pgngame->blackElo, tagbuffer));
    appendToString(&string, generateRoasterLine("ECO", pgngame->eco, tagbuffer));
    appendToString(&string, generateRoasterLine("NIC", pgngame->nic, tagbuffer));
    appendToString(&string, generateRoasterLine("TimeControl", pgngame->timeControl, tagbuffer));
    appendToString(&string, generateRoasterLine("Termination", pgngame->termination, tagbuffer));
...
static int testParsing(void)
{
    ...
    assert(parsePGNMove("d5", &variation->singlePosition) == getPackedMove(D4, D5, NO_PIECE));

    assert(parsePGNMove("b4", &variation->singlePosition) == getPackedMove(B2, B4, NO_PIECE));

    assert(parsePGNMove("bxa3", &variation->singlePosition) == getPackedMove(B2, A3, NO_PIECE));

    assert(parsePGNMove("O-O", &variation->singlePosition) == getPackedMove(E1, G1, NO_PIECE));

    assert(parsePGNMove("O-O-O", &variation->singlePosition) == getPackedMove(E1, C1, NO_PIECE));

    assert(parsePGNMove("Ng3+", &variation->singlePosition) == getPackedMove(E4, G3, NO_PIECE));

    assert(parsePGNMove("Nf6#", &variation->singlePosition) == getPackedMove(E4, F6, NO_PIECE));

    assert(parsePGNMove("Nfg5", &variation->singlePosition) == getPackedMove(F3, G5, NO_PIECE));

    initializeVariation(variation, p2);

    assert(parsePGNMove("h6", &variation->singlePosition) == getPackedMove(H7, H6, NO_PIECE));

    assert(parsePGNMove("f5", &variation->singlePosition) == getPackedMove(F7, F5, NO_PIECE));

    assert(parsePGNMove("fxg6", &variation->singlePosition) == getPackedMove(F7, G6, NO_PIECE));

    assert(parsePGNMove("d1=B", &variation->singlePosition) == getPackedMove(D2, D1, (Piece)BISHOP));

    assert(parsePGNMove("O-O", &variation->singlePosition) == getPackedMove(E8, G8, NO_PIECE));

    assert(parsePGNMove("O-O-O", &variation->singlePosition) == getPackedMove(E8, C8, NO_PIECE));

    assert(parsePGNMove("Nf5", &variation->singlePosition) == getPackedMove(D6, F5, NO_PIECE));

    assert(parsePGNMove("Q2xc3", &variation->singlePosition) == getPackedMove(B2, C3, NO_PIECE));

    assert(parsePGNMove("Qdxc3", &variation->singlePosition) == getPackedMove(D4, C3, NO_PIECE));

    assert(parsePGNMove("Qb4xc3", &variation->singlePosition) == getPackedMove(B4, C3, NO_PIECE));
