...
{
    return (bool)(getDirectAttackers(position, position->king[opponent(position->activeColor)], position->activeColor,
...
{
    return (bool)(getDirectAttackers(position, position->king[position->activeColor], opponent(position->activeColor),
                                     position->allPieces) == EMPTY_BITBOARD);
}
...
int seeMove(Position *position, const Move move)
{
    ...
    attackers[WHITE] = getDirectAttackers(position, to, WHITE, position->allPieces);
    attackers[BLACK] = getDirectAttackers(position, to, BLACK, position->allPieces);
...
{
    qsort(&(movelist->moves[0]), movelist->numberOfMoves, sizeof(Move), compareMoves);
}
...
void initQuiescenceMovelist(Movelist *movelist, Position *position, PlyInfo *plyInfo,
                            MoveHistoryEntry (*moveHistory)[HISTORY_SIZE], int currentPly, const Move hashMove,
                            const int restDepth, const bool check)
{
    ...
    if (check) {
        ...
        movelist->currentStage = (restDepth >= 0 ? MG_SCHEME_QUIESCENCE_WITH_CHECKS : MG_SCHEME_QUIESCENCE);
    }
 ...
 */
void initCheckMovelist(Movelist *movelist, Position *position, MoveHistoryEntry (*moveHistory)[HISTORY_SIZE],
                       int currentPly)
{
    movelist->position = position;
    movelist->plyInfo = 0;
        ...
        if (movelist->nextMove < movelist->numberOfMoves) {
            ...
            case MGS_GOOD_CAPTURES: {
                ...
                if (basicValue[movelist->position->piece[getFromSquare(move)]] >
                    ...
                    movelist->badCaptures[movelist->numberOfBadCaptures++] = move;
            ...
            case MGS_KILLER_MOVES:
                ...
                if (moveIsPseudoLegal(position, killer1) && position->piece[getToSquare(killer1)] == NO_PIECE &&
                    position->piece[getFromSquare(killer1)] == (Piece)getMoveValue(killer1) &&
                ...
                if (moveIsPseudoLegal(position, killer2) && position->piece[getToSquare(killer2)] == NO_PIECE &&
                    position->piece[getFromSquare(killer2)] == (Piece)getMoveValue(killer2) &&
                ...
                if (moveIsPseudoLegal(position, killer3) && position->piece[getToSquare(killer3)] == NO_PIECE &&
                    position->piece[getFromSquare(killer3)] == (Piece)getMoveValue(killer3) &&
                    (pieceType(position->piece[getFromSquare(killer3)]) != PAWN ||
                     getToSquare(killer3) != position->enPassantSquare) &&
                    movesAreEqual(killer3, movelist->hashMove) == FALSE && movesAreEqual(killer3, killer1) == FALSE &&
                ...
                if (moveIsPseudoLegal(position, killer4) && position->piece[getToSquare(killer4)] == NO_PIECE &&
                    position->piece[getFromSquare(killer4)] == (Piece)getMoveValue(killer4) &&
                ...
                if (moveIsPseudoLegal(position, killer5) && position->piece[getToSquare(killer5)] == NO_PIECE &&
                    position->piece[getFromSquare(killer5)] == (Piece)getMoveValue(killer5) &&
                    (pieceType(position->piece[getFromSquare(killer5)]) != PAWN ||
                     getToSquare(killer5) != position->enPassantSquare) &&
                    movesAreEqual(killer5, movelist->hashMove) == FALSE && movesAreEqual(killer5, killer1) == FALSE &&
                    movesAreEqual(killer5, killer2) == FALSE && movesAreEqual(killer5, killer3) == FALSE &&
                ...
                if (moveIsPseudoLegal(position, killer6) && position->piece[getToSquare(killer6)] == NO_PIECE &&
                    position->piece[getFromSquare(killer6)] == (Piece)getMoveValue(killer6) &&
            ...
            case MGS_BAD_CAPTURES:
                ...
                movelist->nextMove = 0;
                memmove(&movelist->moves[0], &movelist->badCaptures[0], movelist->numberOfMoves * sizeof(Move));
                break;
...
bool moveIsPseudoLegal(const Position *position, const Move move)
{
    ...
    if (squareIsValid(from) == FALSE || squareIsValid(to) == FALSE || (piece = position->piece[from]) == NO_PIECE ||
        pieceColor(piece) != position->activeColor) {
        return FALSE;
    }

    if (pieceType(position->piece[from]) == PAWN && colorRank(position->activeColor, from) == RANK_7) {
        if (newPiece != WHITE_QUEEN && newPiece != WHITE_ROOK && newPiece != WHITE_BISHOP && newPiece != WHITE_KNIGHT) {
            return FALSE;
        }
    } else {
    ...
    if (pieceType(position->piece[from]) == PAWN && position->enPassantSquare != NO_SQUARE) {
        Bitboard allPieces = position->allPieces | minValue[position->enPassantSquare];
    ...
    if (pieceType(piece) == KING && hasCastlings(position->activeColor, position->castlingRights)) {
        moves |= getCastlingMoves(position->activeColor, position->castlingRights, position->allPieces);
    }
...
bool moveIsLegal(const Position *position, const Move move)
{
    ...
    if (makeMove(variation, move) == 0 && passiveKingIsSafe(&variation->singlePosition)) {
        result = TRUE;
    }
...
static Bitboard getPinnedPieces(const Position *position, const Color pinningColor)
{
    const Square kingSquare = position->king[opponent(pinningColor)];
    Bitboard pinnedPieces = EMPTY_BITBOARD,
             pinningCandidates = (generalMoves[ROOK][kingSquare] & (position->piecesOfType[QUEEN | pinningColor] |
                                                                    position->piecesOfType[ROOK | pinningColor])) |
                                 (generalMoves[BISHOP][kingSquare] & (position->piecesOfType[QUEEN | pinningColor] |
    ...
    ITERATE_BITBOARD(&pinningCandidates, pinningCandidate)
    {
        ...
        if ((imSquares & position->piecesOfColor[pinningColor]) == EMPTY_BITBOARD) {
            const Bitboard pinnedCandidates = imSquares & position->piecesOfColor[opponent(pinningColor)];
...
int getNumberOfPieceMoves(const Position *position, const Color color, const int sufficientNumberOfMoves)
{
    ...
    Bitboard moves, pieces = position->piecesOfColor[color] &
                             ~(position->piecesOfType[PAWN | color] | minValue[position->king[color]]) & unpinnedPieces;

    ITERATE_BITBOARD(&pieces, square)
    {
        switch (pieceType(position->piece[square])) {
        case QUEEN:
            moves = getMagicQueenMoves(square, position->allPieces) & permittedSquares;
            break;

        case ROOK:
            moves = getMagicRookMoves(square, position->allPieces) & permittedSquares;
            break;

        case BISHOP:
            moves = getMagicBishopMoves(square, position->allPieces) & permittedSquares;
            break;
...
int seeMoveRec(Position *position, const Move move, Bitboard attackers[2], const int minGain)
{
    ...
    if (horizontalDistance(to, from) == verticalDistance(to, from)) {
        attackers[WHITE] |= getMagicBishopMoves(to, position->allPieces) &
                            (position->piecesOfType[WHITE_QUEEN] | position->piecesOfType[WHITE_BISHOP]) &
                            position->allPieces;
        attackers[BLACK] |= getMagicBishopMoves(to, position->allPieces) &
                            (position->piecesOfType[BLACK_QUEEN] | position->piecesOfType[BLACK_BISHOP]) &
                            position->allPieces;
    } else {
        attackers[WHITE] |= getMagicRookMoves(to, position->allPieces) &
                            (position->piecesOfType[WHITE_QUEEN] | position->piecesOfType[WHITE_ROOK]) &
                            position->allPieces;
        attackers[BLACK] |= getMagicRookMoves(to, position->allPieces) &
                            (position->piecesOfType[BLACK_QUEEN] | position->piecesOfType[BLACK_ROOK]) &
                            position->allPieces;
    }

    if (to == position->enPassantSquare && pieceType(position->piece[to]) == PAWN) {
    ...
    if (getNewPiece(move) != NO_PIECE) {
        valueCaptured += basicValue[getNewPiece(move) | activeColor] - basicValue[PAWN | activeColor];
    ...
    } else if (attackers[passiveColor] & position->piecesOfType[KNIGHT | passiveColor]) {
        leastValuableAttacker = (Piece)(KNIGHT | passiveColor);
    } else if (attackers[passiveColor] & position->piecesOfType[BISHOP | passiveColor]) {
        leastValuableAttacker = (Piece)(BISHOP | passiveColor);
    } else if (attackers[passiveColor] & position->piecesOfType[ROOK | passiveColor]) {
        leastValuableAttacker = (Piece)(ROOK | passiveColor);
    } else if (attackers[passiveColor] & position->piecesOfType[QUEEN | passiveColor]) {
        leastValuableAttacker = (Piece)(QUEEN | passiveColor);
    } else {
        leastValuableAttacker = (Piece)(KING | passiveColor);
    }

    leastValuableAttackers = attackers[passiveColor] & position->piecesOfType[leastValuableAttacker];
...
Gameresult getGameresult(Variation *variation)
{
    ...
    if (pos->numberOfPawns[WHITE] == 0 && pos->numberOfPawns[BLACK] == 0 && hasWinningPotential(pos, WHITE) == FALSE &&
    ...
    for (i = 0; i < movelist->numberOfMoves; i++) {
        if ((movelist->moves[i] & 0xFFFF) == shortMove) {
            ...
            if (i < movelist->numberOfMoves) {
                memmove(&movelist->moves[i], &movelist->moves[i + 1], (movelist->numberOfMoves - i) * sizeof(Move));
            }
...
bool listContainsSimpleMove(Movelist *movelist, const Square from, const Square to)
{
...
static void addMoveByValue(Movelist *movelist, const Move move)
{
    ...
    assert(insertPosition == 0 || value <= getMoveValue(movelist->moves[insertPosition - 1]));

    if (insertPosition < movelist->numberOfMoves) {
        assert(value > getMoveValue(movelist->moves[insertPosition]));

        memmove(&movelist->moves[insertPosition + 1], &movelist->moves[insertPosition],
...
void addMoveAtPosition(Movelist *movelist, const Move move, const int insertPosition)
{
    assert(insertPosition >= 0);
    assert(insertPosition <= movelist->numberOfMoves);
...
void deleteMoveAtPosition(Movelist *movelist, const int position)
{
    ...
    if (position < movelist->numberOfMoves) {
        memmove(&movelist->moves[position], &movelist->moves[position + 1],
...
{
    if (movelist->moveHistory == NULL) {
        return 0;
    }

    const int idx = historyIndex(move, position);
    int ply = movelist->currentPly;
    const MoveHistoryEntry *entry = &movelist->moveHistory[ply][idx];
...
static void addCaptures(Movelist *movelist, const Position *position, const Square from, Bitboard captures)
{
    ...
    ITERATE_BITBOARD(&captures, to)
    {
        ...
        if (i < movelist->numberOfMoves) {
            memmove(&movelist->moves[i + 1], &movelist->moves[i], (movelist->numberOfMoves - i) * sizeof(Move));
        }
...
static void addPromotions(Movelist *movelist, const Square from, Bitboard moves)
{
    Square to;
    ...
    ITERATE_BITBOARD(&moves, to)
    {
        ...
        movelist->badCaptures[movelist->numberOfBadCaptures++] = getMove(from, to, WHITE_ROOK, value);

        value = promotionMoveSortValue(movelist->position, to, WHITE_BISHOP);
        movelist->badCaptures[movelist->numberOfBadCaptures++] = getMove(from, to, WHITE_BISHOP, value);

        value = promotionMoveSortValue(movelist->position, to, WHITE_KNIGHT);
        movelist->badCaptures[movelist->numberOfBadCaptures++] = getMove(from, to, WHITE_KNIGHT, value);
    }
}
...
void generateSpecialMoves(Movelist *movelist)
{
    ...
    Bitboard moves = EMPTY_BITBOARD, pieces, pawnCaptureTargets, promotionPawns;
    ...
    {
        moves = getPawnCaptures((Piece)(PAWN | activeColor), from, pawnCaptureTargets) |
    ...
    if (position->activeColor == WHITE) {
        const Bitboard capturingPawns = ((pawnCaptureTargets & nonA) >> 9) | ((pawnCaptureTargets & nonH) >> 7);

        pieces &= capturingPawns;
    } else {
        const Bitboard capturingPawns = ((pawnCaptureTargets & nonA) << 7) | ((pawnCaptureTargets & nonH) << 9);
    ...
    movelist->movesOfPiece[movelist->numberOfPieces].square = position->king[activeColor];
    movelist->movesOfPiece[movelist->numberOfPieces++].moves = moves;
    moves &= captureTargets;
...
void generateRestMoves(Movelist *movelist)
{
    ...
    Bitboard pieces =
        (activeColor == WHITE
             ...
             : position->piecesOfType[BLACK_PAWN] & ~((position->allPieces << 8) | squaresOfRank[RANK_2]));
    Square from;
    const Square k1from = (movelist->killer1Executed ? getFromSquare(movelist->plyInfo->killerMove1) : NO_SQUARE);
    const Square k2from = (movelist->killer2Executed ? getFromSquare(movelist->plyInfo->killerMove2) : NO_SQUARE);
    const Square k3from = (movelist->killer3Executed ? getFromSquare(movelist->plyInfo->killerMove3) : NO_SQUARE);
    const Square k4from = (movelist->killer4Executed ? getFromSquare(movelist->plyInfo->killerMove4) : NO_SQUARE);
    const Square k5from = (movelist->killer5Executed ? getFromSquare(movelist->plyInfo->killerMove5) : NO_SQUARE);
    const Square k6from = (movelist->killer6Executed ? getFromSquare(movelist->plyInfo->killerMove6) : NO_SQUARE);
    int i;

    ITERATE_BITBOARD(&pieces, from)
    {
        Bitboard moves = getPawnAdvances(activeColor, from, position->allPieces);
...
void generateChecks(Movelist *movelist, bool allChecks)
{
    ...
    Bitboard pieces = position->piecesOfType[PAWN | activeColor] & ~promotionCandidates[activeColor];
    ...
    permittedSquares = (allChecks ? ~position->piecesOfColor[activeColor] : ~position->allPieces);
    ...
    for (i = 0; i < movelist->numberOfPieces; i++) {
        ...
        if (testSquare(orthoChecks, from)) {
            ...
            Bitboard batteryPiece = position->piecesOfColor[activeColor] & squaresBehind[from][opponentKing] &
                                    getMagicRookMoves(from, position->allPieces);
            batteryPieceSquare = getLastSquare(&batteryPiece);

            if (batteryPieceSquare != NO_SQUARE &&
                (int)(position->piece[batteryPieceSquare] & pieceProperties) != pieceProperties) {
        ...
        if (piece == KING) {
            Bitboard castlingMoves = moves & ~getKingMoves(position->king[activeColor]);
            Square kingSquare;

            ITERATE_BITBOARD(&castlingMoves, kingSquare)
            {
                const Bitboard obstacles = position->allPieces & maxValue[position->king[activeColor]];
                Bitboard rookMoves;
                const Square rookSquare = (Square)((kingSquare + position->king[activeColor]) / 2);
...
void generateEscapes(Movelist *movelist)
{
    ...
    Bitboard attackers = getDirectAttackers(position, kingsquare, passiveColor, position->allPieces), defenders;
    Bitboard kingmoves = getKingMoves(kingsquare) & ~position->piecesOfColor[activeColor];
    Bitboard corridor;
    ...
    ITERATE_BITBOARD(&defenders, from)
    {
        if (pieceType(position->piece[from]) == PAWN && testSquare(promotionCandidates[activeColor], from)) {
            ...
            value = promotionMoveSortValue(movelist->position, to, WHITE_ROOK);
            movelist->moves[movelist->numberOfMoves++] = getMove(from, to, WHITE_ROOK, value);

            value = promotionMoveSortValue(movelist->position, to, WHITE_BISHOP);
            movelist->moves[movelist->numberOfMoves++] = getMove(from, to, WHITE_BISHOP, value);
    ...
    if (position->enPassantSquare != NO_SQUARE && attackerType == PAWN) {
        defenders = getPawnCaptures((Piece)(PAWN | opponent(activeColor)), position->enPassantSquare,
        ...
        ITERATE_BITBOARD(&defenders, from)
        {
            movelist->moves[movelist->numberOfMoves++] = getMove(from, to, newPiece, value);
        }
...
sortMoves:
    ...
    assert(movelist->hashMove == NO_MOVE || movesAreEqual(movelist->moves[0], movelist->hashMove));
}
...
bool simpleMoveIsCheck(const Position *position, const Move move)
{
    ...
    const Bitboard moves = getMoves(to, position->piece[from], position->allPieces);
    ...
    if (testSquare(getMagicRookMoves(target, position->allPieces), from)) {
        Bitboard batteryPieces =
            (position->piecesOfType[ROOK | attackingColor] | position->piecesOfType[QUEEN | attackingColor]) &
            squaresBehind[from][target];
        Square square;

        ITERATE_BITBOARD(&batteryPieces, square)
        {
            Bitboard blockingPieces =
                (position->allPieces | minValue[to]) & squaresBetween[target][square] & maxValue[from];
    ...
    if (testSquare(getMagicBishopMoves(target, position->allPieces), from)) {
        Bitboard batteryPieces =
            (position->piecesOfType[BISHOP | attackingColor] | position->piecesOfType[QUEEN | attackingColor]) &
            squaresBehind[from][target];
        Square square;
...
bool kingCanEscape(Position *position)
{
    ...
    {
        if (getDirectAttackers(position, square, passiveColor, obstacles) == EMPTY_BITBOARD) {
            return TRUE;
        }
    }
...
int initializeModuleMovegeneration(void)
{
    ...
    promotionPieceValue[WHITE_KNIGHT] = promotionPieceValue[BLACK_KNIGHT] = 2 - VALUEOFFSET_BAD_MOVE;
...
static int testPseudoLegalMoves(void)
{
    ...
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E2, E4, NO_PIECE)));
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(G1, F3, NO_PIECE)));
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(G1, E2, NO_PIECE)) == FALSE);
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(G8, F6, NO_PIECE)) == FALSE);
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E4, E5, NO_PIECE)) == FALSE);

    attackers = getDirectAttackers(&variation->singlePosition, F3, WHITE, variation->singlePosition.allPieces);
    ...
    attackers = getDirectAttackers(&variation->singlePosition, C6, BLACK, variation->singlePosition.allPieces);
    ...
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E2, E4, NO_PIECE)) == FALSE);
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(G1, F3, NO_PIECE)) == FALSE);
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(G1, E2, NO_PIECE)) == FALSE);
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(G8, F6, NO_PIECE)));
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E4, E5, NO_PIECE)) == FALSE);
    ...
    attackers = getDirectAttackers(&variation->singlePosition, D2, WHITE, variation->singlePosition.allPieces);
    ...
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E4, E5, NO_PIECE)));

    makeMove(variation, getPackedMove(E4, E5, NO_PIECE));

    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(D7, D5, NO_PIECE)));

    makeMove(variation, getPackedMove(D7, D5, NO_PIECE));

    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E5, D6, NO_PIECE)));
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(G1, F3, NO_PIECE)));

    makeMove(variation, getPackedMove(G1, F3, NO_PIECE));

    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E7, E6, NO_PIECE)));

    makeMove(variation, getPackedMove(E7, E6, NO_PIECE));

    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(F1, C4, NO_PIECE)));
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E1, G1, NO_PIECE)) == FALSE);

    makeMove(variation, getPackedMove(F1, C4, NO_PIECE));

    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(F8, C5, NO_PIECE)));
    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E8, G8, NO_PIECE)) == FALSE);

    makeMove(variation, getPackedMove(F8, C5, NO_PIECE));

    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E1, G1, NO_PIECE)));

    makeMove(variation, getPackedMove(E1, G1, NO_PIECE));

    assert(moveIsPseudoLegal(&variation->singlePosition, getPackedMove(E8, G8, NO_PIECE)));
...
static int testLegalMoves(void)
{
    ...
    initializeVariation(variation, FEN_GAMESTART);
    assert(moveIsLegal(&variation->singlePosition, getPackedMove(E2, E4, NO_PIECE)));
    assert(moveIsLegal(&variation->singlePosition, getPackedMove(G1, F3, NO_PIECE)));

    makeMove(variation, getPackedMove(E2, E4, NO_PIECE));

    assert(moveIsLegal(&variation->singlePosition, getPackedMove(G8, F6, NO_PIECE)));

    makeMove(variation, getPackedMove(D7, D5, NO_PIECE));
    makeMove(variation, getPackedMove(F1, B5, NO_PIECE));

    assert(moveIsLegal(&variation->singlePosition, getPackedMove(B8, C6, NO_PIECE)));
    assert(moveIsLegal(&variation->singlePosition, getPackedMove(B8, D7, NO_PIECE)));
    assert(moveIsLegal(&variation->singlePosition, getPackedMove(B8, A6, NO_PIECE)) == FALSE);
    assert(moveIsLegal(&variation->singlePosition, getPackedMove(G8, F6, NO_PIECE)) == FALSE);
...
static int testStaticExchangeEvaluation(void)
{
    ...
    assert(seeMove(&variation->singlePosition, move) ==
           basicValue[BLACK_PAWN] - basicValue[WHITE_BISHOP] + basicValue[BLACK_KNIGHT]);
    ...
    assert(seeMove(&variation->singlePosition, move) ==
           basicValue[BLACK_PAWN] + basicValue[BLACK_KNIGHT] - basicValue[WHITE_ROOK]);
    ...
    assert(seeMove(&variation->singlePosition, move) < 0);
    move = getPackedMove(A5, E5, NO_PIECE);
    assert(seeMove(&variation->singlePosition, move) == basicValue[BLACK_PAWN] - basicValue[WHITE_ROOK]);
    ...
    assert(seeMove(&variation->singlePosition, move) == PAWN_FOR_BISHOP);
    move = getPackedMove(A5, E5, NO_PIECE);
    assert(seeMove(&variation->singlePosition, move) == basicValue[BLACK_PAWN] - basicValue[WHITE_KNIGHT] ||
           seeMove(&variation->singlePosition, move) == basicValue[BLACK_PAWN] - basicValue[WHITE_BISHOP]);
    move = getPackedMove(F6, F3, NO_PIECE);
    assert(seeMove(&variation->singlePosition, move) == basicValue[WHITE_KNIGHT] - basicValue[BLACK_QUEEN]);
    ...
    assert(seeMove(&variation->singlePosition, move) == basicValue[WHITE_ROOK] - basicValue[BLACK_KNIGHT]);
...
static int testStaticExchangeEvaluationWithKing(void)
{
    ...
    assert(seeMove(&variation->singlePosition, move) == basicValue[BLACK_PAWN] - basicValue[WHITE_QUEEN]);
    ...
    assert(seeMove(&variation->singlePosition, move) == basicValue[BLACK_PAWN] - basicValue[WHITE_BISHOP]);
...
static int testStaticExchangeEvaluationWithEnPassantCapture(void)
{
    ...
    assert(seeMove(&variation->singlePosition, move) == basicValue[BLACK_ROOK] - basicValue[WHITE_PAWN]);
...
{
    static const char *fen1 = "r1b1k2r/ppp2ppp/2n2n2/1B6/1b2qp2/2NP1N2/PPP1QPPP/R3K2R w KQkq - 0 9";
    Variation *variation = aligned_alloc(64, sizeof(Variation));
    if (variation == NULL) {
